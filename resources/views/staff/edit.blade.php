...
<div style="max-width: 672px; margin: 0 auto; display: flex; flex-direction: column; gap: 24px;">
    <div style="display: flex; align-items: center; gap: 16px;">
        ...
        <div>
            <h1 style="font-size: 24px; font-weight: 700; color: white; letter-spacing: -0.02em; margin: 0;">Edit Staff</h1>
            <p style="color: rgba(255,255,255,0.5); font-size: 14px; margin: 4px 0 0 0;">Perbarui informasi akun staff.</p>
        </div>
    </div>
        ...
        <div class="glass-card" style="padding: 24px;">
            <div style="display: flex; flex-direction: column; gap: 24px;">
                <div>
                    ...
                    <input type="text" name="nama_user" class="form-input-custom @error('nama_user') error @enderror" placeholder="Masukkan nama lengkap" value="{{ old('nama_user', $user->nama_user) }}" required>
                    @error('nama_user') <div class="form-error">{{ $message }}</div> @enderror
                </div>

                <div>
                    <label class="form-label">Email</label>
                    <input type="email" name="email" class="form-input-custom @error('email') error @enderror" placeholder="nama@rentscar.id" value="{{ old('email', $user->email) }}" required>
                ...
                <div>
                    ...
                    <div style="position: relative;">
                        <input type="password" name="password" id="passwordField" class="form-input-custom @error('password') error @enderror" placeholder="Masukkan password baru">
                ...
                <div style="padding-top: 24px; border-top: 1px solid rgba(255,255,255,0.05); display: flex; justify-content: flex-end; gap: 12px;">
                    ...
                    <button type="submit" class="btn-glass" style="background: #C1121F; color: white; border: none; box-shadow: 0 0 24px -6px rgba(193,18,31,0.6); padding: 10px 16px;">
                        <i class="bi bi-check-lg" style="margin-right: 8px;"></i> Simpan Perubahan
                    </button>
                </div>
            </div>
        </div>
